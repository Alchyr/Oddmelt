...
namespace Oddmelt.OddmeltCode.Nodes;

public partial class NStitchCardHolder : NCardHolder, IPoolable
{
    private static MegaCrit.Sts2.Core.Logging.Logger Logger { get; } = new("StitchHolder", LogType.Generic);
    ...
    private static readonly List<NStitchCardHolder> _activeHolders = [];
    ...
    public NStitchCardHolder Initialize(NCard card)
    {
        ...
        SetCard(card);
        _lockPosition = false;
        card.CardHighlight?.AnimHide(); //null before ready
        Scale = StitchedScale;
    ...
    protected override void CreateHoverTips()
    {
        ...
        var nHoverTipSet = NHoverTipSet.CreateAndShow(this, CardNode.Model.HoverTips);
    ...
    public void StitchedModelChanged(CardModel? stitched)
    {
        ...
        var cardNode = NCard.FindOnTable(stitched);
        if (cardNode != null)
        {
